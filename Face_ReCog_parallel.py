...
# Parallelization attempt using the `multiprocessing` module

# def eig_calc(matrix):
#     """Returns the eigen values and eigen vectors of `matrix`"""
#     vals, vecs = np.linalg.eig(matrix)
#     return vals, vecs

# if __name__ == '__main__':
#     pool = mp.Pool(mp.cpu_count())
#     vals, vecs = [pool.map(eig_calc, image_cov)]
#     pool.close()
#     pool.join()


# Parallelization attempt usinng the `threading` module
...
# thread = Thread(target=eig_calc, args=(image_cov,))
# thread.start()
# thread.join()


# CuPy Parallelization attempt
# gpuin_image_cov = cuda.to_device(image_cov)
# gpuout_image_cov = cuda.device_array_like(image_cov)
# vals, vecs = cp.linalg.eigh(image_cov)
# image_cov = gpuout_image_cov.copy_to_host()

# Numba Parallelization attempt
# @cuda.jit(device=True)
# @vectorize([float64(float64)], target="cuda")
# @njit
# @jit
# def eig_vals(matrix):
#     """Returns the eigen values of `matrix`"""
...
# @jit
# def eig_vecs(matrix):
#     """Returns the eigen vectors of `matrix`"""
#     _,vecs = np.linalg.eig(matrix)
#     return vecs

# vals = eig_vals(image_cov)
# vecs = eig_vecs(image_cov)

# Ray parallelization attempt
# @timebudget
# def run_complex_operations(operation, input):
#     ray.get(operation, input)
...
# vals = run_complex_operations(eig_vals, image_cov)
# vecs = run_complex_operations(eig_vecs, image_cov)
